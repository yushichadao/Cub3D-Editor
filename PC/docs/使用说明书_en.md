...
1. While reading the manual, select any text:
   ...
   - 📱 **Web · touch**: long press shows a selection anchor and two draggable handles; drag them to scan-select the text. To stay compatible with domestic ROMs (MIUI / ColorOS / vivo etc.) the "super drag" and the native long-press menu are disabled, so the system copy/share bar no longer appears — instead an in-app "sticky note / highlight / copy" overlay takes over;
