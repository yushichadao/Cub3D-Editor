...
Start-Process -FilePath '.\dist\win-unpacked\cub3d-editor.exe' -ArgumentList @('--no-proxy-server','--proxy-server=direct://','--disable-background-networking') -PassThru | Out-Null
Start-Sleep -Seconds 12

Write-Host "=== PROTO (first 2 lines = gap check) ==="
if (Test-Path $proto) {
