...
| 实验隔离 | 所有论证 demo、截图、实验数据进 `lab/`，**严禁**进 `web/ pc/ android/` 等产品面 |
...
| `manager/` | ~~`admin/`~~ | 管理中枢已随更新系统撤除，归档至 `_retired/admin/` |
| `shared/` | `shared/` | 单一可信源，不变 |
| `tools/` | `tools/` | 维护脚本，保留 |
| `wiki/` | `wiki/` | 文档，保留 |
| `downloads/` | `_retired/downloads/` | 发布产物已归档（更新系统撤除） |
...
| 语义化 | 见名知义：`sync-shared.mjs`、`check-i18n-keys.mjs`、`version.json` |
...
| 作用域 | 约定 | 示例 |
|--------|------|------|
| JS 变量/函数 | camelCase | `currentTheme`、`applyHighlights`、`saveStore` |
| 常量（配置/环境） | UPPER_SNAKE（env）或 SCREAMING | `BOOTV`、`__CUB3D_LONG__` |
| CSS 变量 | kebab，前缀 `--` | `--accent`、`--bg-deep` |
| 主题 key | 小写 camel/kebab | `neon` `forest` `themeForest` |
| 构建注入常量 | 双下划线包裹 | `__CUB3D_PACKAGED__` `__CUB3D_LONG__` |
...
about.long.label        = "构建日期"
env.warn.modal.title    = "当前环境警告"
```
...
| `develop` | 集成开发 |
| `feat/<scope>` | 新功能，`<scope>` 用 kebab（如 `feat/text-tool`） |
| `fix/<scope>` | 修复 |
...
| 变量名 | UPPER_SNAKE，语义化：`GH_TOKEN` `DEPLOY_HOST` `ANDROID_KEY_ALIAS` |
| 禁止硬编码 | 代码内不得出现真实密钥（凭证统一由 `secrets/.env` 管理） |
| 路径配置外置 | 站点域名、IP、分流规则集中维护，不散落三端 |
...
├── shared/          # 单一可信源（i18n 索引 / scripts / language / docs）
├── web/             # Web 客户端（原 Web/）
├── pc/              # PC 客户端（原 PC/，electron-builder 26）
├── android/         # Android 客户端（原 Android/，Capacitor 6）
├── _retired/        # 已撤除模块归档（admin / deploy / downloads / 更新相关）
├── tools/           # 维护脚本（sync-shared / 语言包 / 校验）
...
1. **管理中枢目录名**：`manager/` 曾改为 `admin/`，后随更新系统整体撤除并归档至 `_retired/admin/`。
...
- 根散落 `auto-card.png`
- ~~`manager/server.mjs` + `tools/publish-updates.mjs` 内联百度翻译真实凭证~~（已随管理端归档至 `_retired/`，如复用需自查清理凭证）
- ~~`manager/server.mjs` 硬编码 `DEFAULT_ADMIN_TOKEN`~~（同上，已归档）
- `env-android.mjs` / `setup-android-sdk.ps1` 写死 `C:\Users\yushi\dev-tools`
- ~~三端 `HOSTS` 数组逐字重复 + 安卓 `http://` 明文假回退~~（随更新系统撤除，已无此问题）
