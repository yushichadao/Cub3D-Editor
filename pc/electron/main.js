...
// 打包后的 Electron 会继承系统/用户的 NODE_OPTIONS（例如 IDE 注入的
// --require=.../node-language-shim.cjs）。打包应用不支持该变量，却仍会
// 尝试加载该外部模块（路径常含中文），卡约 30 秒超时后才处理首个
// app:// 请求，表现为启动进程长时间无窗口。必须在 require('electron')
// 之前清掉，避免打包启动被外部环境变量拖死。
...
// GPU 着色器磁盘缓存：%APPDATA% 下目录创建失败时 GPU 进程会卡住/白屏。
// 禁用磁盘缓存并允许 SwiftShader 回退，避免启动被 GPU 缓存拖死。
