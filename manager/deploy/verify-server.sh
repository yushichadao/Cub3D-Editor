...
# 1) 后端存在
[ -f "$MGR/server.mjs" ] && check "manager/server.mjs 存在" 0 || check "manager/server.mjs 存在" 1
...
if [ -f "$ENVFILE" ]; then
  ...
  elif echo "$tok" | grep -qi "请改成\|YushiCub3D@2026"; then check "ADMIN_TOKEN 仍是默认/占位值(需改!)" 1
...
if [ -d "$ROOT/.git" ]; then
  ...
  echo "      对齐命令(服务器): cd $ROOT && git fetch && git checkout 7e9253a && sudo bash manager/deploy/deploy.sh"
else
