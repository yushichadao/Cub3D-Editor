...
// 密钥集中（改进 G）：统一从仓库内 secrets/.env 读取，落实密钥集中、真实值不入库。
// 轻量自写 .env 加载（不引入 dotenv 依赖），找不到文件仅告警，不阻断本地开发。
function loadSecretsEnv() {
  const envPath = path.join(ROOT, 'secrets', '.env');
  if (!fs.existsSync(envPath)) {
    console.warn('[secrets] 未找到 secrets/.env，使用环境变量/默认值（生产部署务必配置）。');
    return;
  }
  try {
    ...
    console.log('[secrets] 已从 secrets/.env 加载密钥配置。');
  } catch (e) {
    console.warn('[secrets] 读取 secrets/.env 失败：', e.message);
  }
}
loadSecretsEnv();
...
// 管理后台登录令牌（即后台密码）。必须来自环境变量 ADMIN_TOKEN（secrets/.env 或部署环境）。
// 不再内置硬编码默认令牌，避免密钥散落（原 DEFAULT_ADMIN_TOKEN 硬编码已移除）。
const DEFAULT_ADMIN_TOKEN = process.env.ADMIN_TOKEN || '';
...
// ---------- 百度翻译（更新话术多语言化，仅管理端，不进客户端运行时）----------
// 凭证：必须来自环境变量 BAIDU_APPID / BAIDU_KEY（secrets/.env 集中管理，真实值不入库）。
// 已移除原硬编码的工程内置凭证，杜绝密钥散落（见 secrets/.env.example）。
...
async function baiduTranslate(text, to){
  if (!BAIDU_APPID || !BAIDU_KEY) {
    throw new Error('百度翻译未配置：请在 secrets/.env 设置 BAIDU_APPID / BAIDU_KEY');
  }
  const from = 'zh';
...
// 管理后台登录：账号 + 当前有效令牌（令牌来自 ADMIN_TOKEN 环境变量/后台修改写入；代码不再内置默认值）
...
app.listen(PORT, '0.0.0.0', async () => {
  ...
  if (!process.env.ADMIN_TOKEN) console.log('[提示] 未配置 ADMIN_TOKEN：后台登录将拒绝（请通过 secrets/.env 或环境变量设置强令牌）。');
  console.log('========================================');
});
